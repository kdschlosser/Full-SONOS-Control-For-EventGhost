...
eg.RegisterPlugin(
    ...
    canMultiLoad=False,
    description=(
        "This plugin allows you to control your SONOS zone players. This works"
        " with grouped zones or stereo pairs. This plugin will search your "
        "network for Zone Players during startup, and if any SONOS ZP is added"
        " or removed from the network the plugin will automatically update "
        "itself. Each ZP is unique based on the MAC address. This means even "
        "if the name of a ZP is changed it won't affect your actions. If you "
        "have to replace a ZP, then all actions that use that ZP will have to "
        "be updated. Many more comands will be added soon."
    ),
...
################################### Globals ###################################
# dict to store ZP objects.
globalZPList = {}

# dict to store service/subscription objects
globalServiceList = {}
globalDebug = 0
globalRestartScheduler = None

# make sure to include dual \\ and include it at the end.
albumartloc = "C:\\SONOS_Covers\\"

######################## Manually add IP address below ########################
# this plugin is designed to support PCs with only one active network card
# So to guarantee the right IP address is selected when you have multiple
# network cards enter it below.
# Note that this PC should have a static address if an address is entered
# below. To allow dynamic ip address in which the plugin gets the IP address
# automatically, leave it blank ("")
...
### example of calling the speech plugin:
# eg.plugins.Speech.TextToSpeech(
#   'Microsoft Hazel Desktop - English (Great Britain)',
#   1,
#   'Today is a beutiful day. the temperature is 87 degrees outside at {TIME}',
#   0,
#   100
# )
...
def HtmlSplit(data=""):
    ...
    try:
        ...
        eg.TriggerEvent("ERROR", prefix='SONOS', payload=trigger)
    for s in headerlist:
        # some text is upper case and other is not, so this will force it to
        # be lower in case it changes or is different for different devices.
        variable = s.split(":", 1)[0] # .lower()
        try:
            # remove white space, there is usually a space after :
...
class AsyncRequesting(asyncore.dispatcher):
    ...
    def start_connection(self):
        ...
        # it appears that if you call eg.RestartAsyncore() to close together,
        # it causes all kinds of problems with the asyncore and sockets. To fix
        # this, eg.RestartAsyncore() is only called once 0.1 seconds has passed
        # without any other new sockets created.
        try:
            self.renewCallBack = eg.scheduler.CancelTask(
                globalRestartScheduler
            )
        except:
            pass
        globalRestartScheduler = eg.scheduler.AddTask(
            .4,
            self.RestartSonosAsyncore
        )
        # eg.RestartAsyncore()
        try:
            ...
            self.HOSTzp, self.PORTzp, e)
            trigger = "Connection to {} on port {} failed: {}".format(
                self.HOSTzp,
                self.PORTzp,
                e
            )
    ...
    def handle_close(self):
        if globalDebug >= 2:
            print "{} -- CLOSED handle_close, State: {}".format(
                self.portused,
                self.connectionMade
            )
        ...
        if self.connectionMade == "CONNECTED": # connection made but then closed, so retry.
            ...
            # if globalDebug >= 1:
            print (
                "{} socket closed before reading, RETRYING connection".format(
                    self.portused
                )
            )
            ...
            if globalDebug >= 1:
                print "{} -- handle_close state:, {}".format(
                    self.portused,
                    self.connectionMade
                )
            self.close()
        else: # everything else and "NO"
            data = "No response from {}:{}".format(self.HOSTzp, self.PORTzp)
            if globalDebug >= 1:
                print data
            self.close()
    ...
    def handle_read(self):
        ...
        try:
            ...
            if data != '':
                ...
                if self.transferEncoding == "":
                    ...
                    if self.response['body'][-4:] == "\r\n\r\n":
                        if self.connectionClose == 'close':
                            ...
                            self.close()
                        self.response['body'] = self.chunk_decode(
                            self.response['body'])
                        self.callback(self.response)
                ...
                self.portused, len(self.response['body']), self.contentLength,
                self.response)
                self.callback(self.response)
            ...
            if data != '':
                ...
                try:
                    self.transferEncoding = self.response[
                        'Transfer-Encoding'] # =============================
                except:
                    try:
                        self.transferEncoding = self.response[
                ...
                try:
                    self.contentLength = int(self.response[
                        'Content-Length']) # =============================
                except:
                    try:
    ...
    def handle_expt(self):
        ...
        try:
            ...
            pass
        trigger = "ERROR, handle_expt: " + str(self.HOSTzp) + ":" + str(
            self.PORTzp)
        eg.TriggerEvent("ERROR", prefix='SONOS', payload=trigger)
        raise Exception(
            "ERROR, handle_expt: %s:%s" % (self.HOSTzp, self.PORTzp))
    ...
    def handle_error(self):
        ...
        try:
            ...
            pass
        PrintException()
        trigger = "ERROR, handle_error: " + str(self.HOSTzp) + ":" + str(
            self.PORTzp)
        eg.TriggerEvent("ERROR", prefix='SONOS', payload=trigger)
        raise Exception(
            "ERROR, handle_error: %s:%s" % (self.HOSTzp, self.PORTzp))

    def handle_connect_expt(self, expt):
        errorText = "connection error: %s \r\n..... %s:%s" % (
        expt, self.HOSTzp, self.PORTzp)
        eg.TriggerEvent("ERROR", prefix='SONOS', payload=errorText)
        try:
            self.close()
        except:
            pass
        trigger = "ERROR, handle_connect_expt: " + str(
            self.HOSTzp) + ":" + str(self.PORTzp)
        eg.TriggerEvent("ERROR", prefix='SONOS', payload=trigger)
        raise Exception(
            "ERROR, handle_connect_expt: %s:%s" % (self.HOSTzp, self.PORTzp))
...
class Service():
    ...
    def ServiceErrorHandler(self, data):
        if "ERROR" in data.keys():
            if data['ERROR'].find("No Response") >= 0:
                ...
                eg.TriggerEvent("ZonePlayerDeleted", prefix='SONOS',
                    payload=trigger)
                if globalDebug >= 1:
                    print "\n\n"
                if self.url.find(
                    "ZoneGroupTopology") > 0: # if true, subscribe to a different ZP.
                    try:
                        uuid = globalZPList.keys()[
                            0] # select random ZP to get grouptopology from
                        tempservice = Service(globalZPList[uuid],
                            "/ZoneGroupTopology/Event", ZoneGroupTopologyEvent)
                    except:# if there are no more ZPs, through error
                        trigger = "ERROR: Can't connect to SONOS Zone Players"
                        eg.TriggerEvent("ERROR", prefix='SONOS',
                            payload=trigger)
    ...
    def Renew(self):
        ...
        self.subrequest = AsyncRequesting(self.zp.ip, port, data,
            self.SubResponse)
        ...
        if globalDebug >= 1:
            print "%s requesting renew: %s - %s" % (
            self.subrequest.portused, globalZPList[self.zp.uuid].name,
            self.url.split("/")[-2])
            # eg.RestartAsyncore()
    ...
    def UnsubResponse(self, data):
        ...
        global globalZPList
        if globalDebug >= 2:
            print "%s Unsubscribed response %s - %s" % (
            self.unsubrequest.portused, globalZPList[self.zp.uuid].name,
            self.url.split("/")[-2])
    ...
    def Unsubscribe(self):
        ...
        try:
            ...
            if globalDebug >= 2:
                print "can't Cancel Task renewCallBack in Unsubscribed %s" % str(
                    e)
            pass
        try:
        ...
        data = "UNSUBSCRIBE " + self.url + " HTTP/1.1\r\nUSER-AGENT: Linux UPnP/1.0 Sonos/24.0-69180m (WDCR:Microsoft Windows NT 6.2.9200.0)\r\nHOST: " + self.zp.ip + ":" + str(
            port) + "\r\nSID: " + self.SID + "\r\n\r\n"
        if globalDebug >= 2:
            print "sending unsubscribe request: %s - %s" % (
            globalZPList[self.zp.uuid].name, self.url.split("/")[-2])
        self.unsubrequest = AsyncRequesting(self.zp.ip, port, data,
            self.UnsubResponse)
    ...
    def SubResponse(self, data):
        ...
        try:
            ...
            try:
                ...
                pass
            self.subsTimeout = int(
                response['TIMEOUT'].split("-")[1]) # ex: TIMEOUT: Seconds-3200
            if globalDebug >= 2:
                print response['SID']
            self.SID = response['SID']
            globalServiceList[
                self.SID] = self # store service object in global dict
            # store SID info in ZP object
            globalZPList[self.zp.uuid].services[
                self.url.split("/")[-2]] = self.SID
            if globalZPList[self.zp.uuid].name == "":
                globalZPList[self.zp.uuid].name = self.zp.ip
            if globalDebug >= 2:
                print "Subscribed to %s on %s" % (
                self.url.split("/")[-2], globalZPList[self.zp.uuid].name)
            if globalDebug >= 2:
                print "%s subscribed response %s - %s" % (
                self.subrequest.portused, globalZPList[self.zp.uuid].name,
                self.url.split("/")[-2])
            self.renewCallBack = eg.scheduler.AddTask(self.subsTimeout / 2,
                self.Renew)
        except Exception, e:
    ...
    def Subscribe(self):
        ...
        data = "SUBSCRIBE " + self.url + " HTTP/1.1\r\nNT: upnp:event\r\nTIMEOUT: Second-" + str(
            self.subsTimeout) + "\r\nHOST: " + self.zp.ip + ":" + str(
            port) + "\r\nCALLBACK: <http://" + localip + ":" + str(
            self.eventport) + "/events>\r\nContent-Length: 0\r\n\r\n"
        self.subrequest = AsyncRequesting(self.zp.ip, port, data,
            self.SubResponse)
        if globalZPList[self.zp.uuid].name == "":
            globalZPList[self.zp.uuid].name = self.zp.ip
        if globalDebug >= 2:
            print "%s sending subscribe request: %s - %s" % (
            self.subrequest.portused, globalZPList[self.zp.uuid].name,
            self.url.split("/")[-2])
...
def RenderingControlEvent(uuid, data):
    ...
    try:
        ...
        xmlLastChange = xml.getElementsByTagName('LastChange')[
        ...
        if True:
            ...
            for volume in xml.getElementsByTagName('Volume'):
                if volume.attributes[
                    'channel'].value == "Master": # if master channel, save current volume
                    vol = volume.attributes['val'].value
                    if globalZPList[
                        uuid].volume != vol: # if volume has changed, trigger event and save.
                        trigger = "%s.%s" % (uuid, "Volume")
                        vol = "vol-%s %s" % (vol, globalZPList[uuid].name)
                        eg.TriggerEvent(trigger, prefix='SONOS', payload=vol)
                        globalZPList[uuid].volume = volume.attributes[
                            'val'].value
        # except:
        #    pass
        try:
        ...
        try:
            ...
            for mute in xml.getElementsByTagName('Mute'):
                if mute.attributes[
                    'channel'].value == "Master": # if master channel, save current mute
                    vol = mute.attributes['val'].value
                    if globalZPList[
                        uuid].mute != vol: # if mute has changed, trigger event and save.
            ...
            pass
        try:
            globalZPList[uuid].outputFixed = \
            xml.getElementsByTagName('OutputFixed').attributes['val'].value
        except:
            pass
        try:
            globalZPList[uuid].headphoneConnected = \
            xml.getElementsByTagName('HeadphoneConnected').attributes[
                'val'].value
        except:
            pass
    except Exception, e:
...
def UpdateTrackTitle(uuid, title, artist="", source="",
    albumarturl=""): # added 2/3/2016
    # updated all Zone Players within a group.
    for key in globalZPList:
        if not globalZPList[key].invisible:
            if globalZPList[key].coordinator == uuid:
...
def AVTransportEvent(uuid, data):
    ...
    try:
        ...
        try:
            transportstate = \
            xml.getElementsByTagName("TransportState")[0].attributes[
                'val'].value
            # check to see if avTransportURI exists, if it doesn't update transportstate (only occurs when first starting)
            if globalDebug >= 1:
                print "%s Received ZP Transport State: %s" % (
                globalZPList[uuid].name, transportstate)
        except:
            pass
        ...
        try:
            newAVTransportURI = \
            xml.getElementsByTagName("AVTransportURI")[0].attributes[
                'val'].value
            ...
            try:
                ...
                pass
        try:
            globalZPList[uuid].currentPlayMode = \
            xml.getElementsByTagName("CurrentPlayMode")[0].attributes[
                'val'].value
            if globalDebug >= 1:
                print "%s currentPlayMode: %s" % (
                globalZPList[uuid].name, globalZPList[uuid].currentPlayMode)
        except:
            pass
        try:
            globalZPList[uuid].playbackStorageMedium = \
            xml.getElementsByTagName("PlaybackStorageMedium")[0].attributes[
                'val'].value
            if globalDebug >= 1:
                print "%s playbackStorageMedium: %s" % (
                globalZPList[uuid].name,
                globalZPList[uuid].playbackStorageMedium)
        except:
            pass
        ...
        try:
            ...
            xml.getElementsByTagName("AVTransportURIMetaData")[0].attributes[
                'val'].value
            if avtransportURIMetaData == "":
                if globalDebug >= 1:
                    print "%s avtransportURIMetaData: <empty>" % globalZPList[
                        uuid].name
            else:
                ...
                source = tempxml.getElementsByTagName("dc:title")[
                    0].firstChild.nodeValue
                if globalDebug >= 1:
                    print "%s Stream Title: %s" % (
                    globalZPList[uuid].name, source)
        except Exception, e:
            if globalDebug >= 2:
        ...
        try:
            ...
            xml.getElementsByTagName("CurrentTrackMetaData")[0].attributes[
                'val'].value
            ...
            albumartlink = tempxml.getElementsByTagName("upnp:albumArtURI")[
                0].firstChild.nodeValue
            albumarturl = "http://" + globalZPList[
                uuid].ip + ":1400" + albumartlink # 2/3/2016
            if globalDebug >= 2:
            ...
            pass
        try:
            title = tempxml.getElementsByTagName("dc:title")[
                0].firstChild.nodeValue
            if globalDebug >= 1:
                print "Track: " + title
        except:
            pass
        try:
            artist = tempxml.getElementsByTagName("dc:creator")[
                0].firstChild.nodeValue
            if globalDebug >= 1:
                print "Artist: " + artist
        except:
            pass
        try:
            album = tempxml.getElementsByTagName("upnp:album")[
                0].firstChild.nodeValue
            if globalDebug >= 1:
                print "Album: " + album
            ...
            pass
        # trigger EG events
        UpdateTrackTitle(uuid, title, artist, source,
            albumarturl) # add 2/3/2016
    except Exception, e:
...
def ZoneGroupTopologyEvent(uuid, data):
    ...
    try:
        ...
        try:
            xmlstring = xml.getElementsByTagName('ZoneGroupState')[
                0].firstChild.nodeValue
        except:
            if globalDebug >= 1:
        ...
        for zg in grouplist:
            ...
            try:
                ...
                for zp in zplist:
                    ...
                    if globalDebug >= 1:
                        print '{0: <27}'.format(uuid) + '{0: <17}'.format(ip) + \
                              attrlist['ZoneName']
            except:
                pass
            ...
            for zp in zplist:
                ...
                if globalZPList[uuid].coordinator != coordinator:
                    if globalDebug >= 1:
                        print "%s coordinator changed to %s" % (
                        globalZPList[uuid].name,
                        globalZPList[coordinator].name)
                    if coordinator == uuid:
                ...
                if coordinator == uuid: # if true, ZP is coordinator
                    stringflagco = " :coordinator -%s-" % \
                                   globalZPList[uuid].services["AVTransport"]
                    if globalZPList[uuid].services[
                        "AVTransport"] == "": # no SID
                        if globalDebug >= 2:
                            print "    Subscribing to %s" % globalZPList[
                                uuid].name
                        subscribeList.append(uuid)
                    else:
                        pass
                        if globalDebug >= 2:
                            print "    AVTransport already subscribed to %s" % \
                                  globalZPList[uuid].name
                else:
                    stringflagco = " -%s-" % globalZPList[uuid].services[
                        "AVTransport"]
                    if not globalZPList[uuid].services[
                        "AVTransport"] == "": # SID present
                        unsubscribeList.append(uuid)
                        if globalDebug >= 2:
                            print "    Unsubscribing to %s" % globalZPList[
                                uuid].name
                            # globalServiceList[globalZPList[uuid].services["AVTransport"]].Unsubscribe()
                    else:
                        if globalDebug >= 2:
                            print "    AVTransport already unsubscribed to %s" % \
                                  globalZPList[uuid].name
                if "Invisible" in attrlist:
                    ...
                    if globalDebug >= 1:
                        print '{0: <27}'.format(uuid) + '{0: <17}'.format(ip) + \
                              attrlist[
                                  'ZoneName'] + " (Invisible)" + stringflagco
                else:
                    globalZPList[uuid].invisible = 0
                    if globalDebug >= 1:
                        print '{0: <27}'.format(uuid) + '{0: <17}'.format(ip) + \
                              attrlist['ZoneName'] + stringflagco
        for zp in subscribeList:
            if not globalZPList[zp].isZoneBridge == '1':
                if globalDebug >= 1:
                    print "Subscribing to AVTransportEvent on %s" % \
                          globalZPList[zp].name
                # globalZPList[zp].services["AVTransport"] = "testing"
                tempservice = Service(globalZPList[zp],
                    "/MediaRenderer/AVTransport/Event", AVTransportEvent,
                    serverPort)
        for zp in unsubscribeList:
            if globalDebug >= 1:
                print "Unsubscribing to AVTransportEvent on%s" % globalZPList[
                    zp].name
            # globalZPList[zp].services["AVTransport"] = ""
            globalServiceList[
                globalZPList[zp].services["AVTransport"]].Unsubscribe()
...
class EventChannel(asyncore.dispatcher):
    ...
    def handle_read(self):
        ...
        try:
            if self.contentlength == 0:
                ...
                self.contentlength = int(
                    self.eventdata['CONTENT-LENGTH']) # CONTENT-LENGTH
                if self.eventdata[
                    'NT'] != 'upnp:event': # NT #check to make sure it's an event
    ...
    def handle_expt(self):
        errorText = "EventChannel handle_expt : %s:%s" % (
        self.HOSTzp, self.PORTzp)
        eg.TriggerEvent("ERROR", prefix='SONOS', payload=errorText)
        try:
            self.close()
        except:
            pass
        raise Exception("ERROR, EventChannel handle_expt: %s:%s" % (
        self.HOSTzp, self.PORTzp))

    def handle_error(self):
        errorText = "EventChannel handle_error : %s:%s" % (
        self.HOSTzp, self.PORTzp)
        eg.TriggerEvent("ERROR", prefix='SONOS', payload=errorText)
        try:
            self.close()
            ...
            pass
        PrintException()
        raise Exception("ERROR, EventChannel handle_error: %s:%s" % (
        self.HOSTzp, self.PORTzp))
    ...
    def handle_connect_expt(self, expt):
        ...
        try:
            ...
            pass
        raise Exception("ERROR, EventChannel handle_connect_expt: %s:%s" % (
        self.HOSTzp, self.PORTzp))


class EventServer(asyncore.dispatcher):
    def __init__(self,
        port=0):# leaving the port assigned to 0 will allow the OS to pick an available port.
        asyncore.dispatcher.__init__(self)
        self.port = port
        self.create_socket(socket.AF_INET, socket.SOCK_STREAM)
    ...
    def handle_expt(self):
        errorText = "EventServer handle_expt : %s:%s" % (
        self.HOSTzp, self.PORTzp)
        eg.TriggerEvent("ERROR", prefix='SONOS', payload=errorText)
        try:
            self.close()
            ...
            pass
        raise Exception("ERROR, EventServer handle_expt: %s:%s" % (
        self.HOSTzp, self.PORTzp))

    def handle_error(self):
        errorText = "ERROR, EventServer handle_error : %s:%s" % (
        self.HOSTzp, self.PORTzp)
        eg.TriggerEvent("ERROR", prefix='SONOS', payload=errorText)
        try:
            self.close()
            ...
            pass
        PrintException()
        raise Exception("ERROR, EventServer handle_error: %s:%s" % (
        self.HOSTzp, self.PORTzp))
    ...
    def handle_connect_expt(self, expt):
        ...
        try:
            ...
            pass
        raise Exception("ERROR, EventServer handle_connect_expt: %s:%s" % (
        self.HOSTzp, self.PORTzp))
...
class ZonePlayer():
    ...
    def __init__(self, uuid, ip, xmllocation):
        ...
        services = {
            ...
            'ContentDirectory':  "",
            # none right now, otherwise all ZPs (bridge?)
            'AVTransport':       "", # Play etc. only the coordinators
            'AlarmClock':        "", # only one ZP
            'RenderingControl':  ""
            # vol etc. All ZPs except if invisible (sub, or stereo pair)
        }
    ...
    def GetDeviceXMLResponse(self, response):
        try:
            if response['status'] == "OK":
                ...
                self.model = xml.getElementsByTagName('modelNumber')[
                    0].firstChild.nodeValue
                self.modelName = xml.getElementsByTagName('modelName')[
                    0].firstChild.nodeValue
                self.name = xml.getElementsByTagName('roomName')[
                    0].firstChild.nodeValue
                self.swversion = xml.getElementsByTagName('softwareVersion')[
                    0].firstChild.nodeValue
                self.hwversion = xml.getElementsByTagName('hardwareVersion')[
                    0].firstChild.nodeValue
                if globalDebug >= 1:
                    print "%s--%s--%s is a %s" % (
                    self.name, self.uuid, self.ip, self.modelName)
                if globalDebug >= 1:
                    print "      %s--HW Version:  %s" % (
                    self.name, self.hwversion)
                    print "      %s--SW Version:  %s" % (
                    self.name, self.swversion)
                    print "      %s--SONOS Model: %s" % (self.name, self.model)
            else:
                print "**Response Device XML Response HTML Error from %s: %s %s" % (
                self.name, response['status-code'], response['status'])
                if globalDebug >= 1:
    ...
    def GetDeviceXML(self):
        ...
        self.deviceXMLRequest = AsyncRequesting(self.ip, port, data,
            self.GetDeviceXMLResponse)
    ...
    def TransportState(self, transportstate):
        ...
        trigger = ""
        if globalDebug >= 1:
            print "%s Pre Transport State: %s" % (
            self.name, self.transportState)
    ...
    def SendCommandWait(self):
        while True:
            time.sleep(
                0.1)# small delay to allow socket to be created and wait between reads
            try:
    ...
    def SendCommand(self, service, command, device='',
        variables={'InstanceID': '0'}, responseCallBack='StandardCmdRes'):
        ...
        try:
            command = command[0].upper() + command[
            1:] # make sure first character is capitalized
            if globalDebug >= 1:
            ...
            data = self.CommandPacketForm(path, hostport, xml, service,
                command)
            if globalDebug >= 2:
                print data
            self.socketSendCmd = AsyncRequesting(self.ip, self.COMMANDPORT,
                data, callBackFunction)
            # eg.RestartAsyncore() removed
            if globalDebug >= 1:
                print "%s Sending %s to %s" % (
                self.socketSendCmd.portused, command, self.name)
        except Exception, e:
            print "ERROR Handled: Sending %s to %s Failed - %s" % (
            command, self.name, e)
            self.sendCmdState = "ready"
            errorText = "ERROR Handled: Sending %s to %s Failed - %s" % (
            command, self.name, e)
            eg.TriggerEvent("ERROR", prefix='SONOS', payload=errorText)

    def SendCommandPrint(service, command, device='',
        variables={'InstanceID': '0'}):
        self.SendCommand(service, command, device, variables,
            'StandardCmdResPrint')
    ...
    def SendSetZoneName(self, name):
        ...
        self.SendCommand("DeviceProperties", "SetZoneAttributes", "",
            variables)

    # ------------- AVTransport Commands -------------------------------------------------

    def SendBecomeCoordinatorOfStandaloneGroup(self):
        self.SendCommand('AVTransport', 'BecomeCoordinatorOfStandaloneGroup',
            'MediaRenderer')

    def SendPlay(self):
        self.SendCommand("AVTransport", "Play", "MediaRenderer",
            variables={'InstanceID': '0', 'Speed': '1'})
    ...
    def SendGetPositionInfo(self, callback=None):
        ...
        self.SendCommand("AVTransport", "GetPositionInfo", "MediaRenderer",
            variables, 'ResponseGetPositionInfo')
    ...
    def SendGetMediaInfo(self, callback=None):
        ...
        self.SendCommand("AVTransport", "GetMediaInfo", "MediaRenderer",
            variables, 'ResponseGetMediaInfo')
    ...
    def SendSetAVTransportURI(self, uri, urimetadata=''):
        ...
        self.SendCommand("AVTransport", "SetAVTransportURI", "MediaRenderer",
            variables)
    ...
    def SendAddURIToQueue(self, uri, urimetadata):
        ...
        self.SendCommand("AVTransport", "AddURIToQueue", "MediaRenderer",
            variables)

    def SendSetPlayMode(self, playmode):
        variables = {'InstanceID': '0', 'NewPlayMode': playmode}
        self.SendCommand("AVTransport", "SetPlayMode", "MediaRenderer",
            variables)
    ...
    def SendSetCrossfadeMode(self, crossfade):
        ...
        self.SendCommand("AVTransport", "SetCrossfadeMode", "MediaRenderer",
            variables)

    def SendRemoveAllTracksFromQueue(self):
        variables = {'InstanceID': '0'}
        self.SendCommand("AVTransport", "RemoveAllTracksFromQueue",
            "MediaRenderer", variables)
    ...
    def SendConfigureSleepTimer(self, timer): # timer in minutes
        ...
        self.SendCommand("AVTransport", "ConfigureSleepTimer", "MediaRenderer",
            variables)
    ...
    def SendRelVolume(self, incvol):
        ...
        self.SendCommand("RenderingControl", "SetRelativeVolume",
            "MediaRenderer", variables)

    def SendSetVolume(self, setvol):
        variables = {'InstanceID': '0', 'Channel': 'Master', 'DesiredVolume': setvol}
        self.SendCommand("RenderingControl", "SetVolume", "MediaRenderer",
            variables)

    def SendMuteOn(self):
        variables = {'InstanceID': '0', 'Channel': 'Master', 'DesiredMute': '1'}
        self.SendCommand("RenderingControl", "SetMute", "MediaRenderer",
            variables)
    ...
    def SendGroupRelVolume(self, incvol):
        ...
        self.SendCommand("GroupRenderingControl", "SetRelativeGroupVolume",
            "MediaRenderer", variables)

    def SendSetGroupVolume(self, setvol):
        variables = {'InstanceID': '0', 'DesiredVolume': setvol}
        self.SendCommand("GroupRenderingControl", "SetGroupVolume",
            "MediaRenderer", variables)

    def SendGroupMuteOff(self):
        variables = {'InstanceID': '0', 'DesiredMute': '0'}
        self.SendCommand("GroupRenderingControl", "SetGroupMute",
            "MediaRenderer", variables)
    ...
    def SendGetFavPlayList(self, callback=None):
        ...
        self.SendCommand("ContentDirectory", "Browse", "MediaServer",
            variables, 'ResponseGetFavPlayList')
    ...
    def StandardCmdResPrint(self, response):
        ...
        try:
            if response['status'] == "OK":
                ...
                print "**Response HTML Error from %s: %s %s" % (
                self.name, response['status-code'], response['status'])
                if globalDebug >= 1:
    ...
    def ResponseGetPositionInfo(self, response):
        ...
        try:
            if response['status'] == "OK":
                ...
                try:
                    self.trackNum = \
                    xml.getElementsByTagName('Track')[0].childNodes[
                        0].nodeValue
                except:
                    self.trackNum = ""
                try:
                    self.trackDuration = \
                    xml.getElementsByTagName('TrackDuration')[0].childNodes[
                        0].nodeValue
                except:
                    self.trackDuration = ""
                try:
                    self.trackURI = \
                    xml.getElementsByTagName('TrackURI')[0].childNodes[
                        0].nodeValue
                except:
                    self.trackURI = ""
                try:
                    self.trackMetaData = \
                    xml.getElementsByTagName('TrackMetaData')[0].childNodes[
                        0].nodeValue
                except:
                    self.trackMetaData = ""
                try:
                    self.relTime = \
                    xml.getElementsByTagName('RelTime')[0].childNodes[
                        0].nodeValue
                except:
                    self.relTime = ""
                if globalDebug >= 1:
    ...
    def ResponseGetMediaInfo(self, response):
        ...
        try:
            if response['status'] == "OK":
                if globalDebug >= 1:
                    print "%s GetMediaInfo Response %s" % (
                    self.name, response['body'])
                xmlstr = response['body']
                xml = parseString(xmlstr.encode('utf-8'))
                try:
                    self.nrTracks = \
                    xml.getElementsByTagName('NrTracks')[0].childNodes[
                        0].nodeValue
                except:
                    self.nrTracks = 0
                try:
                    self.currentURI = \
                    xml.getElementsByTagName('CurrentURI')[0].childNodes[
                        0].nodeValue
                except:
                    self.currentURI = ""
                try:
                    self.currentURIMetaData = \
                    xml.getElementsByTagName('CurrentURIMetaData')[
                        0].childNodes[0].nodeValue
                except:
                    self.currentURIMetaData = ""
                self.isPlayList = (
                self.currentURI.find("x-rincon-queue:") == 0)
                if globalDebug >= 1:
    ...
    def ResponseGetFavPlayList(self, response):
        ...
        try:
            if response['status'] == "OK":
                ...
                xmlplayliststring = xml.getElementsByTagName('Result')[
                    0].firstChild.nodeValue
                ...
                for item in xml.getElementsByTagName('item'):
                    tempdescription = \
                    item.getElementsByTagName('r:description')[
                        0].firstChild.nodeValue.replace(" Station", "")
                    temptitle = item.getElementsByTagName('dc:title')[
                        0].firstChild.nodeValue
                    tempuri = item.getElementsByTagName('res')[
                        0].firstChild.nodeValue
                    subxmlstr = item.getElementsByTagName('r:resMD')[
                        0].firstChild.nodeValue
                    subxml = parseString(subxmlstr.encode('utf-8'))
                    tempurimetadata = subxml.getElementsByTagName('DIDL-Lite')[
                        0].toxml()
                    tempupnpclass = subxml.getElementsByTagName('upnp:class')[
                        0].firstChild.nodeValue
                    if tempupnpclass.find(
                        "playlistContainer") > 0: # is this a streaming station or a playlist
                        tempisplaylist = True
                    else:
                        tempisplaylist = False
                    self.favPlayList[temptitle] = {} # clear/create dict
                    self.favPlayList[temptitle][
                        'description'] = tempdescription
                    self.favPlayList[temptitle]['uri'] = tempuri
                    self.favPlayList[temptitle][
                        'urimetadata'] = tempurimetadata
...
def searchforsonos(broadcastip='239.255.255.250'):
    try:
        ...
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        s.settimeout(
            1.5)# very important if useing while loop to receive all responses,if this is removed, loop occurs
        s.bind((
        localip, 0)) # port changed to 0 to allow OS to pick available port
        ...
        while True:# look until timeout # this is to capture all responses
            ...
            if not data:
                break
            if data.find(
                "ST: urn:schemas-upnp-org:device:ZonePlayer:1") > 0: # verify it's SONOS
                srv_addr = srv_sock[0]
                html = HtmlSplit(data)
...
class Sonos(eg.PluginBase):
    ...
    def closesockets(self):
        ...
        for uuid in globalZPList:
            if globalZPList[uuid].coordinator == uuid:
                ...
                coordinator = ""
            if globalDebug >= 1:
                print uuid + " = " + globalZPList[
                    uuid].name + " : " + coordinator + " SID:" + \
                      globalZPList[uuid].services['AVTransport']
        try:
            if globalDebug >= 1:
    ...
    def Configure(self, debugLvL=0):
        ...
        mySizer.Add(DropDown, (1, 0), (1, 2),
            flag=(wx.ALIGN_CENTER | wx.ALIGN_RIGHT))
...
class WindowUUIDText():
    def __init__(self, value='',
        title="Select a Zone Player from the list below...", text1="",
        text2=""):
...
class WindowUUIDSelectText():
    def __init__(self, uuid="", value='',
        ...
        try:
            p = ChoiceList.index(
                globalZPList[uuid].name + "-" + uuid + "-" + globalZPList[
                    uuid].ip)
...
class WindowUUIDSelecttwo():
    def __init__(self, uuid="", uuid2='',
        title="Select a Zone Player and item from the list below...", text1="",
        text2="", models=[]):
        ...
        try:
            p = ChoiceList.index(
                globalZPList[uuid2].name + "-" + uuid2 + "-" + globalZPList[
                    uuid2].ip)
...
class WindowUUIDSelectList():
    def __init__(self, uuid="", item='',
        title="Select a Zone Player and item from the list below...", list=[],
        text1="", text2=""):
...
class WindowUUIDSelectRange():
    def __init__(self, uuid="", value=0,
        title="Select a Zone Player and Value from the list below...",
        range="0:100", text1="", text2=""):
...
class WindowUUIDMultiSelect():
    def __init__(self, checkList=[],
        title="Select Zone Players from the list below..."):
        panel = eg.ConfigPanel()
        zpCount = 0
        selectedList = []
        ChoiceList = []
        ...
        for k in ChoiceList:
            ...
            mySizer.Add(ZPCheckBox[zpCount], (zpCount + 2, 0),
                flag=wx.ALIGN_LEFT)
            zpCount = zpCount + 1
        ...
        while panel.Affirmed():
            for box in ZPCheckBox:
                if box.GetValue():
                    if box.GetLabel().split("-")[
                        -2] not in selectedList: # only appended if not already added, need for testing command in config window
                        selectedList.append(box.GetLabel().split("-")[
                            -2]) # save only the uuid from the label and only the ZPs checked.
                else:
                    try:
                        selectedList.remove(box.GetLabel().split("-")[
                            -2]) # removes from list if added, this is needed when testing command in config window
                    except:
                        pass
            panel.SetResult(
                selectedList
...
class WindowUUIDSelect():
    def __init__(self, uuid="",
        title="Select a Zone Player from the list below...", models=[]):
        panel = eg.ConfigPanel()
...
class VolumeAdjust(eg.ActionBase):
    ...
    def __call__(self, uuid="", incvol="3"):
        ...
        if globalDebug >= 1:
            print "adjusted volume by " + str(incvol) + " in " + globalZPList[
                uuid].name
    ...
    def Configure(self, uuid="", incvol="3"):
        ...
        winSelectGUI = WindowUUIDSelectRange(uuid, incvol, title, range1,
            text1, text2)
...
class VolumeSet(eg.ActionBase):
    ...
    def __call__(self, uuid="", setvol="20"):
        ...
        # invisible ZPz are not listed. So this should not be a problem
        if globalDebug >= 1:
            print "set volume to " + str(setvol) + " in " + globalZPList[
                uuid].name
    ...
    def Configure(self, uuid="", setvol="20"):
        ...
        text2 = ''''''
        winSelectGUI = WindowUUIDSelectRange(uuid, setvol, title, range1,
            text1, text2)
...
class GroupVolumeAdjust(eg.ActionBase):
    ...
    def __call__(self, uuid="", incvol="3"):
        ...
        # invisible ZPz are not listed. So this should not be a problem
        if globalDebug >= 1:
            print "adjusted group volume by " + str(incvol) + " in " + \
                  globalZPList[uuid].name
...
class GroupVolumeSet(eg.ActionBase):
    ...
    def __call__(self, uuid="", setvol="20"):
        ...
        # invisible ZPz are not listed. So this should not be a problem
        if globalDebug >= 1:
            print "set group volume to " + str(setvol) + " in " + globalZPList[
                uuid].name
...
class SetPlayMode(eg.ActionBase):
    ...
    def __call__(self, uuid="", playmode="NORMAL"):
        ...
        # invisible ZPz are not listed. So this should not be a problem
        if globalDebug >= 1:
            print "set playmode to " + playmode + " in " + globalZPList[
                uuid].name
    ...
    def Configure(self, uuid="", playmode="NORMAL"):
        ...
        winSelectGUI = WindowUUIDSelectList(uuid, playmode, title, list1,
            text1, text2)
...
class SetCrossfade(eg.ActionBase):
    ...
    def __call__(self, uuid="", crossfade="OFF"):
        ...
        if globalDebug >= 1:
            print "set crossfade to " + crossfade + " in " + globalZPList[
                uuid].name
    ...
    def Configure(self, uuid="", crossfade="OFF"):
        ...
        text2 = ''''''
        winSelectGUI = WindowUUIDSelectList(uuid, crossfade, title, list1,
            text1, text2)
...
class SetSleepTimer(eg.ActionBase):
    ...
    def __call__(self, uuid="", timer="30"):
        ...
        if globalDebug >= 1:
            print "set Sleept timer to " + str(timer) + " in " + globalZPList[
                uuid].name
    ...
    def Configure(self, uuid="", timer="30"):
        ...
        winSelectGUI = WindowUUIDSelectRange(uuid, timer, title, range1, text1,
            text2)
...
class GroupZPs(eg.ActionBase):
    ...
    def __call__(self, coordinator="", selectedList=[]):
        ...
        if coordinator == "":
            ...
            for cord, cnt in groups.iteritems():
                ...
                if globalZPList[
                    cord].transportState == "Playing":# top priority to groups that have music playing
                    groups[cord] = groups[cord] + 1000
                if not globalZPList[
                    cord].avTransportURI == "": # priority to groups that have music queued
            ...
            if coordinator != globalZPList[coordinator].coordinator:
                ...
                globalZPList[
                    coordinator].SendBecomeCoordinatorOfStandaloneGroup()
            ...
            # only needs to run if coordinator is currently coordinator of a group.
                ...
                for uuid, zpObj in globalZPList.iteritems():
                    if coordinator == globalZPList[uuid].coordinator and not \
                    globalZPList[uuid].invisible:
    ...
    def Configure(self, coordinator="", selectedList=[]):
        ...
        try:
            p = ChoiceList.index(
                globalZPList[coordinator].name + "-" + coordinator + "-" +
                globalZPList[coordinator].ip)
            ZPDropDown.SetSelection(p)
        except: # ValueError:
            ZPDropDown.SetSelection(0)
        ...
        mySizer.Add(ZPDropDown, (1, 0), (1, 3),
            flag=(wx.ALIGN_TOP | wx.EXPAND))
        ...
        for k in ChoiceList[1:]:
            ...
            if uuid in selectedList:
                ...
                ZPCheckBox[zpCount].SetValue(False)
            mySizer.Add(ZPCheckBox[zpCount], (zpCount + 4, 0),
                flag=wx.ALIGN_LEFT)
            zpCount = zpCount + 1
        ...
        while panel.Affirmed():
            FinalChoice = ZPDropDown.GetStringSelection()
            try:
                coordinator = FinalChoice.split("-")[
                    -2] # save only the uuid from the list.
            except:
                coordinator = ""
            for box in ZPCheckBox:
    ...
    def __call__(self, selectedList={}):
        ...
        for uuid, volDict in selectedList.iteritems():
            if uuid not in globalZPList:
                ...
                globalZPList[uuid].storeMediaInfo['volume'] = globalZPList[
                    uuid].volume
                globalZPList[uuid].storeMediaInfo['mute'] = globalZPList[
                    uuid].mute
                coordinator = globalZPList[uuid].coordinator
                if coordinator not in globalZPList:
                    ...
                    while globalZPList[
                        coordinator].zpTransportState == "TRANSITIONING":
                        time.sleep(0.25)
                    globalZPList[coordinator].StoreMediaInfo()
        ...
        for coordinator in zpList:
            globalZPList[
                coordinator].SendCommandWait() # make sure previous command is complete
        ...
        for uuid, volDict in selectedList.iteritems():
            if uuid not in globalZPList:
                ...
                'vol'] > 0: # if volume set to 0 then use ZP current volume
                globalZPList[uuid].SendCommandWait()
        ...
        for uuid, volDict in selectedList.iteritems():
            if uuid not in globalZPList:
                ...
                'mute']: # if volume set to 0 then use ZP current volume
                globalZPList[uuid].SendCommandWait()
                globalZPList[uuid].SendMuteOff()
    ...
    def Configure(self, selectedList={}):
        ...
        for k in ChoiceList:
            ...
            if uuid in selectedList:
                ...
                volRange[zpCount].SetValue(0)
                muteCB[zpCount].SetValue(False)
            mySizer.Add(volRange[zpCount], (zpCount + 3, 1),
                flag=wx.ALIGN_CENTER)
            mySizer.Add(muteCB[zpCount], (zpCount + 3, 2),
                flag=wx.ALIGN_CENTER)
            mySizer.Add(ZPCheckBox[zpCount], (zpCount + 3, 0),
                flag=wx.ALIGN_LEFT)
            zpCount = zpCount + 1
        ...
        while panel.Affirmed():
            for (box, vol, mute) in zip(ZPCheckBox, volRange, muteCB):
                if box.GetValue():
                    ...
                    selectedList[uuid][
                        'mute'] = mute.GetValue() # save only the uuid from the label and only the ZPs checked.
                else:
...
class NotificationRestore(eg.ActionBase):
    ...
    def __call__(self, restoreDelay=10):
        ...
        globalRestoreTask = eg.scheduler.AddTask(restoreDelay,
            self.RestoreTrackStateNow)
        ...
        for uuid, zpObj in globalZPList.iteritems():
            ...
            if zpObj.storeMediaInfo['mediaRestore']:
                ...
                if zpObj.storeMediaInfo['isPlayList']:
                    zpObj.SendSeek("TRACK_NR",
                        zpObj.storePositionInfo['trackNum'])
                    zpObj.SendCommandWait()
                    zpObj.SendSeek("REL_TIME",
                        zpObj.storePositionInfo['relTime'])
                    zpObj.SendCommandWait()
    ...
    def Configure(self, restoreDelay=10):
        ...
        TextSelect = wx.StaticText(panel, -1, text1)
        try:
            DropDown = wx.SpinCtrl(panel, -1, str(restoreDelay), (0, 0),
                (100, 20))
    ...
    def __call__(self, linein=""):
        ...
        for uuid, zpObj in globalZPList.iteritems():
            if zpObj.storeMediaInfo['mediaRestore']:
                ...
                globalZPList[coordinator].SendPlay()
                if globalDebug >= 1:
                    print "set TrackURI to " + trackuri + " in " + \
                          globalZPList[uuid].name
...
def SendNotificationTrackURI(trackuri):
    ...
    for uuid, zpObj in globalZPList.iteritems():
        if zpObj.storeMediaInfo['mediaRestore']:
            ...
            globalZPList[coordinator].SendCommandWait()
            if globalDebug >= 1:
                print "set TrackURI to " + trackuri + " in " + globalZPList[
                    uuid].name
...
class NotificationGoogleTTS(eg.ActionBase):
    ...
    def __call__(self, tts="", language="en"):
        if len(tts) > 99:
            ...
            text = tts
        trackuri = "http://translate.google.com/translate_tts?ie=UTF-8&q=%s&tl=%s&total=1&idx=0&textlen=%s&prev=input" % (
        text, language, len(text))
        SendNotificationTrackURI(trackuri)
...
class PlayLineIn(eg.ActionBase):
    ...
    def Configure(self, uuid="", linein=""):
        ...
        winSelectGUI = WindowUUIDSelecttwo(uuid, linein, title, text1, text2,
            LINEINMODELS)
...
class PlayTrackURI(eg.ActionBase):
    ...
    def Configure(self, uuid="", trackuri=""):
        ...
        text2 = '''NOTE: track must be accessible by SONOS and in the proper format to play'''
        winSelectGUI = WindowUUIDSelectText(uuid, trackuri, title, text1,
            text2)
...
class StartPlayList(eg.ActionBase):
    ...
    def __call__(self, uuid="", track="", dscr="", uri="", urimetadata="",
        upnpclass="", isplaylist=""):
        if uuid == "":
            print "Please select a ZP to send command to."
            return
        if uuid not in globalZPList:
        ...
        if isplaylist:
            ...
            globalZPList[
                coordinator].SendCommandWait()# wait for previous command to complete
            globalZPList[coordinator].SendAddURIToQueue(uri, urimetadata)
            globalZPList[coordinator].SendSetAVTransportURI(
                "x-rincon-queue:" + coordinator + "#0", "")
            if globalDebug >= 1:
                print "Starting Playlist " + track + " in " + globalZPList[
                    uuid].name
        else:
            globalZPList[coordinator].SendSetAVTransportURI(uri, urimetadata)
            if globalDebug >= 1:
                print "Starting Station " + track + " in " + globalZPList[
                    uuid].name
        globalZPList[
            coordinator].SendCommandWait()# wait for previous command to complete
        globalZPList[coordinator].SendPlay()
    ...
    def Configure(self, uuid="", track="", dscr="", uri="", urimetadata="",
        upnpclass="", isplaylist=""):
        try:
            panel = eg.ConfigPanel()
            ...
            for k, v in globalZPList.iteritems():
                if v.invisible == 0: # find the first non invisible ZP
                    ...
                    break
            if globalDebug >= 0:
                print "Getting Favorites List from %s at %s" % (
                globalZPList[tempuuid].name, globalZPList[tempuuid].ip)
            globalZPList[tempuuid].SendGetFavPlayList(self.ResponseReceived)
            if globalDebug >= 1:
                print "Waiting for SendGetFavPlayList response..."
            globalZPList[
                tempuuid].SendCommandWait()# wait for previous command to complete
            try:
            ...
            TextZPSelect = wx.StaticText(panel, -1,
                "Select a Zone Player from the list below...")
            TextPLSelect = wx.StaticText(panel, -1,
                "Select a Playlist or Station from your SONOS Favorites list...")
            TextPLNote = wx.StaticText(panel, -1,
                '''Make sure to add any station you want to select to the SONOS favorites list before opening this dialogue.\nAlso note when the action is triggered, if it's a playlist it will replace the current queue''')
            # ============ dropdown ==============
            ChoiceList = []
            if globalZPList:
                for k, v in globalZPList.iteritems():
            ...
            PLDropDown = wx.Choice(panel, -1,
                choices=ChoiceList) # (0,0), (60,20)
            # Identify the device and set dropdown to correct position
            p = 0
            try:
            ...
            while panel.Affirmed():
                ...
                panel.SetResult(
                    FinalChoice.split("-")[-2],
                    # save only the uuid from the list.
                    FinalPL,
    ...
    try:
        ...
        for index, woeidElement in enumerate(woeidList):
            ...
            try:
                ...
                pass
            try:
                woeidDict[woeid].append(
                    countryList[index].firstChild.nodeValue)
            except:
                pass
    except Except, e:
...
def GetWeatherForcast(woeid, units):
    ...
    forecast['temp'] = xml.getElementsByTagName('yweather:condition')[
        0].getAttribute('temp')
    forecast['code'] = xml.getElementsByTagName('yweather:condition')[
        0].getAttribute('code')
    try:
        forecast['text'] = CONDITIONCODES[forecast['code']]
    except:
        forecast['text'] = xml.getElementsByTagName('yweather:condition')[
            0].getAttribute('text')
    forecast['todayshigh'] = xml.getElementsByTagName('yweather:forecast')[
        0].getAttribute('high')
    forecast['todayslow'] = xml.getElementsByTagName('yweather:forecast')[
        0].getAttribute('low')
    forecast['todayscode'] = xml.getElementsByTagName('yweather:forecast')[
        0].getAttribute('code')
    try:
        forecast['todaystext'] = CONDITIONCODES[forecast['todayscode']]
    except:
        forecast['todaystext'] = xml.getElementsByTagName('yweather:forecast')[
            0].getAttribute('text')
    return forecast
...
class LocSearchDialog(wx.Dialog):
    def __init__(self, parent, id, title, varwoeid, searchText):
        ...
        if len(ChoiceList) == 0:
            ...
            searchAgain = True
        radioBox = wx.RadioBox(panel, -1, "select your location",
            choices=ChoiceList, style=wx.RA_SPECIFY_COLS, majorDimension=1)
        try:
        ...
        def OKButton(event):
            if globalDebug >= 1:
                print "%s" % woeidList
                print "WOEID: %s Location: %s" % (
                woeidList[radioBox.GetSelection()],
                ChoiceList[radioBox.GetSelection()])
...
class NotificationWeather(eg.ActionBase):
    ...
    def __call__(self, woeid="", location="", units="f"):
        ...
        current = "The current weather is %s degrees and %s" % (
        weather['temp'], weather['text'])
        forecast = "Todays high will be %s degrees and %s" % (
        weather['todayshigh'], weather['todaystext'])
        print current
        print forecast
        ...
        # time.sleep(5)
        trackuri = "http://translate.google.com/translate_tts?ie=UTF-8&q=%s&tl=%s&total=1&idx=0&textlen=%s&prev=input" % (
        forecast, 'en', len(forecast))
        SendNotificationTrackURI(trackuri)

        info = "temp:%s, units:%s, condition:%s" % (
        weather['temp'], units, weather['text'])
        eg.TriggerEvent("CurrentConditions", prefix='WEATHER', payload=info)
        info = "high:%s, low:%s, units:%s, condition:%s" % (
        weather['todayshigh'],
        weather['todayslow'],
        units,
        weather['todaystext']
    ...
    def OnSearchButton(self, event):
        self.selectLocation = LocSearchDialog(None, -1,
            'Select your location...', self.classwoeid,
...
ACTIONS = (
    ...
    (GroupVolumeAdjust, "GroupVolumeAdjust", "GroupVolumeAdjust",
    "GroupVolumeAdjust Sonos.", None),
    (GroupVolumeSet, "GroupVolumeSet", "GroupVolumeSet",
    "GroupVolumeSet Sonos.", None),
    ...
    (SetSleepTimer, "SetSleepTimer", "SetSleepTimer", "Set Sleep Timer Value",
    None),
    (PlayTrackURI, "PlayTrackURI", "PlayTrackURI",
    "Start playing specified track URI", None),
    (PlayLineIn, "PlayLineIn", "PlayLineIn", "Select and Play Line-In", None),
    (PlayTVonPlayBar, "PlayTVonPlayBar", "PlayTVonPlayBar",
    "Select TV input from PlayBar", None),
    (GroupZPs, "GroupZPs", "GroupZPs", "Group Selected Zone Players.", None),
    (UnGroup, "UnGroup", "UnGroup", "Send UnGroup to ZonePlayer.", None),
    (NotificationStart, "NotificationStart", "NotificationStart",
    "Execute before notifications, stores State of ZPs", None),
    (NotificationRestore, "NotificationRestore", "NotificationRestore",
    "Restore State after Notifications", None),
    (NotificationLineIn, "NotificationLineIn", "NotificationLineIn",
    "Play notification through Line-In of a ZP", None),
    (NotificationTrackURI, "NotificationTrackURI", "NotificationTrackURI",
    "Play notification from a location", None),
    (NotificationWeather, "NotificationWeather", "NotificationWeather",
    "Play notification announcing the weather", None),
    (NotificationGoogleTTS, "NotificationGoogleTTS", "NotificationGoogleTTS",
    "Play notification using Google TTS API", None)
